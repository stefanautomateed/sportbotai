...
export class HockeyAdapter extends BaseSportAdapter {
  ...
  /**
   * Get injuries for a hockey team using ESPN's free API
   */
  async getInjuries(teamId: string): Promise<DataLayerResponse<import('../types').NormalizedInjury[]>> {
    try {
      console.log(`[Hockey] Fetching injuries for team: ${teamId}`);

      const injuries = await getESPNInjuriesForTeam(teamId, 'nhl');

      console.log(`[Hockey] Found ${injuries.length} injuries for ${teamId}`);

      return this.success(injuries);
    } catch (error) {
      console.error('[Hockey] Error fetching injuries:', error);
